...
# %% auto 0
__all__ = ['JSON_SCHEMA', 'EXAMPLE_DATA', 'wearable_schema', 'WereableAccessor', 'load_json', 'load_csv', 'WereableData_NEW',
...
class WereableAccessor:
    ...
    def _validate_columns(obj):
        ...
        wereable_cols = ['steps', 'heartrate', 'wake', 'light_estimate', 'activity']
        if not any([col in obj.columns for col in wereable_cols]):
            raise AttributeError("DataFrame have at least one wereable data column (steps, heartrate, wake, light_estimate, or activity).")
    ...
    def _validate_metadata(metadata):
        ...
        if not all([key in metadata.keys() for key in ['data_id', 'subject_id', 'other_info']]):
            raise AttributeError("Metadata only can have `data_id`, `subject_id`, or `other_info` as keys.")
    ...
    def plot(self, 
        ...
        "Plot wereable data"
        if name not in ['steps', 'heartrate', 'wake', 'light_estimate', 'activity']:
            raise AttributeError("Name must be one of steps, heartrate, wake, light_estimate, or activity.")
        if ax is None:
...
def load_json(self,
    ...
    jsonschema.validate(jdict, schema=JSON_SCHEMA)
