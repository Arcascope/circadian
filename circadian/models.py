...
class MelatoninHilaire07(CircadianModel):
    "Implementation of melatonin suppression model from Hilaire's 2007 article 'A physiologically basedA physiologically based mathematical model of melatonin including ocular light suppression and interactions with the circadian pacemaker'"
    def __init__(self, params=None):
        default_params = {
            'taux': 24.2, 'mu': 0.13, 'G': 37.0,
            'beta': 0.0075, 'k': 0.55, 'q': 1.0/3.0,
            'I0': 9500.0, 'p': 0.5, 'alpha_0': 0.05,
            'phi_ref': 0.97, 'cbt_to_dlmo': 7.0,
            'betaIP': 0.047, 'betaCP': 0.025,
            'phi_on': -1.44, 'phi_off': 2.78,
            'delta': 600.0, 'r': 15.36, 'a': 0.0010442}
        num_states = 5 # x, xc, n, H1, H2
        num_inputs = 1 # light
        default_initial_condition = np.array([-1.16307764e-01, -1.21224010e+00,  4.55843062e-01,  3.83411489e-16, 1.67004164e-13])
        super(MelatoninHilaire07, self).__init__(default_params, num_states, num_inputs, default_initial_condition)
        if params is not None:
            self.parameters = params
    ...
    def __str__(self) -> str:
        return "MelatoninHilaire07"

# %% ../nbs/api/00_models.ipynb 73
@patch_to(MelatoninHilaire07)
def derv(self,
         t: float, # time
         state: np.ndarray, # dynamical state (x, xc, n, H1, H2)
         input: np.ndarray # model input (light, wake)
         ) -> np.ndarray: # derivative of the state
     "Right-hand-side of the differential equation model"
     x = state[0,...]
     ...
     dydt[1,...] = np.pi/12 * (self.q * Bhat * xc - x * taux_term)
     dydt[2,...] = 60.0 * (alpha * (1 - n) - self.beta * n)
     dydt[3,...] = - self.betaIP * H1 + A
     dydt[4,...] = self.betaIP * H1 - self.betaCP * H2
